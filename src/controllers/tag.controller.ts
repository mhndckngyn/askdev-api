  ...
  generateDescription: (async (req, res, next) => {
    try {
      ...
      if (!tagName || typeof tagName !== 'string') {
        throw new ApiError(400, "tag.generate-description-tagName-required", true)
      }

      const result = await AIService.generateTagDescription(tagName);

      const resBody: ApiResponse = {
        success: true,
        statusCode: 200,
        message: "tag.generate-description-success",
        content: result
      };
